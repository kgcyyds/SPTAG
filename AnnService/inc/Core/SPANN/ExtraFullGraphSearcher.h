...
extern void solve(void *h_target, void *h_data, std::vector<uint64_t> h_offset, std::vector<int> h_st,
...
namespace SPTAG
{
    namespace SPANN
    {
        ...
        struct Selection
        {
            ...
            Selection(size_t totalsize, std::string tmpdir) : m_tmpfile(tmpdir + FolderSep + "selection_tmp"), m_totalsize(totalsize), m_start(0), m_end(totalsize)
            {
...
#define ProcessPosting()                                                                                          \
    do                                                                                                            \
    {                                                                                                             \
        std::vector<uint64_t> h_offsetVector;                                                                     \
        std::vector<int> h_vectorID;                                                                              \
        std::vector<int> h_st;                                                                                    \
        std::vector<float> h_distance(listInfo->listEleCount);                                                    \
        ...
        {                                                                                                         \
            ...
            h_offsetVector.push_back(offsetVector);                                                               \
            h_vectorID.push_back(vectorID);                                                                       \
        ...
        solve(queryResults.GetQuantizedTarget(), p_postingListFullData, h_offsetVector, h_st, h_distance,         \
              totalBytes, listInfo->listEleCount, vectorDim, typeid(ValueType), sizeof(ValueType));               \
        for (int i = 0; i < listInfo->listEleCount; i++)                                                          \
        {                                                                                                         \
            if (h_st[i])                                                                                          \
                queryResults.AddPoint(h_vectorID[i], h_distance[i]);                                              \
        }                                                                                                         \
    } while (0);
...
*/
                ...
                if (m_enablePostingListRearrange)
                    m_parsePosting = &ExtraFullGraphSearcher<ValueType>::ParsePostingListRearrange;
                else
                    m_parsePosting = &ExtraFullGraphSearcher<ValueType>::ParsePostingList;
                if (m_enableDeltaEncoding)
                    m_parseEncoding = &ExtraFullGraphSearcher<ValueType>::ParseDeltaEncoding;
                else
            ...
            virtual void SearchIndex(int32_t vectorDim, ExtraWorkSpace *p_exWorkSpace,
                                     ...
                                     std::set<int> *truth, std::map<int, std::set<int>> *found)
            {
                ...
                for (uint32_t pi = 0; pi < postingListCount; ++pi)
                {
                    ...
                    request.m_callback = [&p_exWorkSpace, &queryResults, &p_index, &request, &vectorDim, this](bool success)
                    {
                        char *buffer = request.m_buffer;
                        ListInfo *listInfo = (ListInfo *)(request.m_payload);
                        size_t totalBytes = request.m_readSize;

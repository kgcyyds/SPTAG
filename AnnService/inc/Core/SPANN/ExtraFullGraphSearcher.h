...
#define ProcessPosting()                                                                                  \
    ...
    {                                                                                                     \
        ...
        ValueType *p = (ValueType *)(p_postingListFullData + offsetVector);                               \
        vec.insert(vec.end(), p, p + vectorDim * sizeof(ValueType));                                      \
        id.push_back(vectorID);                                                                           \
    }
...
*/
                                     ...
                                     std::set<int> *truth, std::map<int, std::set<int>> *found)
            {
                ...
                for (uint32_t pi = 0; pi < postingListCount; ++pi)
                {
                    ...
                    request.m_callback = [&p_exWorkSpace, &queryResults, &p_index, &request, &vec, &id, &vectorDim, this](bool success)
                    {
